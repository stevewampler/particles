...
object ZeroVector3DFunction extends Vector3DFunction {
  def apply(v: Vector3D) = Vector3D.ZeroValue
}
...
object SinWaveVector3DFunction {
  ...
  //    "frequency":[0.25, 1.0, 1.0],
  //    "phase":[]
  //  }
  def apply(config: JSON): SinWaveVector3DFunction = SinWaveVector3DFunction(
    config.getVector3D("offset").getOrElse(Vector3D.ZeroValue),
    config.getVector3D("amplitude").getOrElse(Vector3D.ZeroValue),
    config.getVector3D("frequency").getOrElse(Vector3D.ZeroValue),
    config.getVector3D("phase").getOrElse(Vector3D.ZeroValue)
  )
}
...
object ParticleTimeFunction {
  def apply(config: JSON): ParticleTimeFunction = ParticleTimeFunction(config.getVector3DFunction("func").getOrElse(ZeroVector3DFunction))
}
...
object Gravity {
  def apply(config: JSON, particlesMap: Map[String, Particle]): Option[List[Force]] = {
    val acc = config.getDouble("acceleration").getOrElse(-9.81)
    config.getStrings("particles").map(names => names.map(particlesMap).map(Gravity(_, acc)))
  }
}
...
object Drag {
  ...
  //    }
  //  }
  def apply(config: JSON, particleMap: Map[String, Particle]): Option[List[Force]] = {
    val fluidDensity = config.getDouble("fluidDensity").getOrElse(0.5)
    val dragCoeff = config.getDouble("dragCoeff").getOrElse(0.47)
    val flowFunc = config.getParticleFunction("flowFunc").getOrElse(ZeroVector3DParticleFunction)
    config.getStrings("particles").map(names => names.map(particleMap).map(Drag(_, fluidDensity, dragCoeff, flowFunc)))
  }
}
case class Drag(
    override val p: Particle,
...
object SpringDamper {
  ...
  //    "springConstant": 5,
  //    "restLength": 10,
  //    "viscousDampingCoeff": 1,
  //    "maxForce": 500
  //  },
  def apply(config: JSON, particleMap: Map[String, Particle]): Option[List[Force]] = {
    val springConstant = config.getDouble("springConstant").getOrElse(1.0)
    val restLength = config.getDouble("restLength").getOrElse(0.0)
    val viscousDampingCoeff = config.getDouble("viscousDampingCoeff").getOrElse(1.0)
    val maxForce = config.getDouble("maxForce").getOrElse(500.0)
    config.getStringsList("particles").map(particleNamesList =>
      particleNamesList.map(particleNames =>
        SpringDamper(
          particleMap(particleNames(0)),
          particleMap(particleNames(1)),
          springConstant,
          restLength,
          viscousDampingCoeff,
          maxForce
